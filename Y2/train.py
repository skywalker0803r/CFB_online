...
'''
此腳本為 Y2 設計，用於訓練預測 DeSOx_2nd 的模型。

主要修改點：
1.  特徵集改用 features2.pkl。
2.  目標變數 (target variable) 改為 DeSOx_2nd。
3.  所有輸出檔案（模型、報告、日誌）將儲存在 Y2 資料夾內，並有 _y2 後綴。
4.  路徑處理改為相對於本檔案位置，確保穩健性。
5.  補回了遺漏的HTML報告生成區塊。
'''
import pandas as pd
...
DEBUG_FILE = os.path.join(SCRIPT_DIR, "train_debug_data_y2.csv")
REPORT_PATH = os.path.join(SCRIPT_DIR, "training_report_y2.html")
DATA_SOURCE = os.path.join(PROJECT_ROOT, 'test0609-CFB2脫硫劑優化改善.feather')

# --- Data Loading and Preprocessing ---
print(f"從 {DATA_SOURCE} 讀取數據...")
df = pd.read_feather(DATA_SOURCE).dropna().tail(10000)
print(f'df讀取完成shape:{df.shape}')
df.index.name = 'datetime'

# select by rule (與Y1相同)
...
# select features for Y2
print(f"從 {FEATURES_PATH} 讀取特徵集...")
with open(FEATURES_PATH, "rb") as f:
    features = pickle.load(f)

# define y_col for Y2
y_col = 'DeSOx_2nd'
print(f"設定目標變數為: {y_col}")
required_cols = features + [y_col]
existing_cols = [col for col in required_cols if col in df.columns]
select_df = df[existing_cols]
...
feature_cols = [f for f in features if f != y_col] + ['prev_target']
...
predictions, abs_errors, thresholds, indices = [], [], [], []
current_model_recent_errors = []
threshold_update_window = 100
percentile_for_threshold = 90
pbar = tqdm(total=total_steps, desc="Y2 動態預測進度")
rebuild_count = 0
current_model = None
i = start_idx
...
while i < len(select_df):
    ...
    test_X = test_row[feature_cols].to_frame().T
    ...
    indices.append(i)
    pbar.update(1)
    error_exceeded_threshold = True if error > current_threshold else False
    i += 1
...
# --- 8. 生成HTML報告 ---
print(f"正在生成HTML報告至 {REPORT_PATH}...")
...
ax.set_title(f'Y2 Actual vs. Predicted Values ({y_col})')
...
html_content = f'''
<!DOCTYPE html><html lang="zh-TW"><head><meta charset="UTF-8"><title>Y2 模型訓練報告</title>
<style>body{{font-family:'Segoe UI',Tahoma,Geneva,Verdana,sans-serif;margin:20px}}h1,h2{{color:#333}}table{{border-collapse:collapse;width:60%;margin-top:20px}}th,td{{border:1px solid #ddd;padding:8px;text-align:left}}th{{background-color:#f2f2f2}}.container{{max-width:1200px;margin:auto}}.summary{{background-color:#eef;padding:15px;border-left:5px solid #66f;margin-top:20px}}img{{max-width:100%;height:auto;margin-top:20px}}</style>
</head><body><div class="container"><h1>Y2 模型訓練報告 ({y_col})</h1>
<h2>模型評估指標</h2>{df_metrics.to_html(index=False, classes='table')}
<h2>模型重建總結</h2><div class="summary"><p>{rebuild_summary}</p></div>
<h2>實際值 vs. 預測值</h2><img src="data:image/png;base64,{image_base64}" alt="Actual vs. Predicted Values Plot">
<h2>預測結果預覽 (前10筆)</h2>{result_df.head(10).to_html(index=False, classes='table')}
<h2>詳細預測數據</h2>{result_df.to_html(index=False, classes='table')}</div></body></html>
'''
